...
function injectContentScript(tabId) {
  chrome.scripting
    ...
    .catch((error) => {
      console.log(`Content script already exists in tab ${tabId}, sending message`);
...
function updateContextMenus() {
  chrome.storage.sync.get(["enabled", "mode", "refreshInterval"], (data) => {
    const settings = {
      ...
      refreshInterval: (typeof data.refreshInterval === "number" ? data.refreshInterval : parseFloat(data.refreshInterval)) || 1
    };
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (
    ...
    chrome.storage.sync.get(["enabled"], (data) => {
      if (data.enabled) {
        console.log(`Monitoring is enabled, ensuring content script is active in tab ${tabId}`);
        ...
        if (isMainPageTab(tab.url)) {
          console.log(`Main page detected, checking monitoring status immediately`);
          setTimeout(() => {
...
function checkTabFor403Error(tabId, url) {
  ...
  chrome.scripting.executeScript({
    ...
    if (results && results[0] && results[0].result && results[0].result.has403Error) {
      console.log(`RaterHub Monitor: 403 error detected in tab ${tabId}, redirecting to main page`);
      redirectTabToMainPage(tabId);
    }
  }).catch((error) => {
  ...
  chrome.scripting.executeScript({
    ...
    if (results && results[0] && results[0].result) {
      ...
      if (!status.isMonitoring && status.shouldMonitor) {
        console.log(`RaterHub Monitor: Main page detected in tab ${tabId}, starting monitoring`);
        startMonitoringOnTab(tabId);
      }
    }
  }).catch((error) => {
    console.log(`Failed to check tab ${tabId} for main page monitoring:`, error);
  });
}

// Function to be executed in the tab context to check 403 errors
function checkPageFor403Error() {
  const has403Error = document.body.innerText.includes("Error 403 Forbidden") || 
...
function checkMainPageMonitoringStatus() {
  ...
  const isMainPage = window.location.href === "https://www.raterhub.com/evaluation/rater";
